// Verify the translate-arm + upper-arm-capsule model for ALL 4 poses (D2/D3 were on fist/
// pointing/victory). Rows = poses; cols = rest / up / left. Shared shoulder S, per-pose elbow AP.
...
(async () => {
  ...
  const cols = [['rest',0,0],['up',0,-MAG],['left',-MAG,0]];
  ...
  await sharp({ create:{ width: cols.length*cw+(cols.length+1)*gap, height: poses.length*ch+(poses.length+1)*gap, channels:4, background:'#141414' } })
    .composite(comp).png().toFile(path.join(dir,'ik_all.png'));
  console.log('ik_all.png (rows: open/fist/pointing/victory; cols: rest / up / left)');
})();
